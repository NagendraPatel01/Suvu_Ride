...
import com.suviridedriver.ui.rides.adapter.CompletedAdpater

class CompletedFragment(val list: List<Data>) : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        if (list.size != 0){
            val onGoingAdpater = CompletedAdpater(requireContext(),list)
            recyclerView!!.layoutManager = LinearLayoutManager(context)
            recyclerView!!.adapter = onGoingAdpater
        }else{
            tvMSG!!.text = "No Completed Ride"

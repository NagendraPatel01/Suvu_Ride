...
class ProfileRepository @Inject constructor(private val APIServices: APIServices) {
    ...
    private val _deleteDriverResponse = MutableLiveData<NetworkResult<SuccessResponse>>()
    val deleteDriverResponse: LiveData<NetworkResult<SuccessResponse>>
        get() = _deleteDriverResponse

    suspend fun deleteDriver() {
        _deleteDriverResponse.postValue(NetworkResult.Loading())

        val response = APIServices.deleteDriver()
        if (response.isSuccessful && response.body() != null) {
            _deleteDriverResponse.postValue(NetworkResult.Success(response.body()!!))
        } else if (response.errorBody() != null) {
            /*val errorObj = JSONObject(response.errorBody()!!.charStream().readText())
  _allScreenLabelResponseLiveData.postValue(NetworkResult.Error(errorObj.getString("message")))*/
            _deleteDriverResponse.postValue(NetworkResult.Error(CommonFunction.getError(response)))
        } else {
            _deleteDriverResponse.postValue(NetworkResult.Error("Something Went Wrong"))
        }
    }
